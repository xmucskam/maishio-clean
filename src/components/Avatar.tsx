...
// ---------------- Viseme → openness map (kept for smoothing/energy)
const VISEME_OPEN: Record<string, number> = {
  X: 0.02, A: 1.00, B: 0.25, C: 0.85, D: 0.75, E: 0.55, F: 0.45, G: 1.00, H: 0.35, L: 0.60,
}
...
const VISEME_RECIPE: Record<string, MouthRecipe> = {
  X: { mouthClose: 0.1, mouthPress: 0.05 },
  A: { jawOpen: 1.0, mouthFunnel: 0.25, mouthUpperUpLeft: 0.2, mouthUpperUpRight: 0.2 },
  B: { jawOpen: 0.35, mouthStretchLeft: 0.6, mouthStretchRight: 0.6, mouthSmileLeft: 0.25, mouthSmileRight: 0.25 },
  C: { jawOpen: 0.25, mouthPucker: 0.9, mouthFunnel: 0.5 },
  D: { jawOpen: 0.55, mouthFunnel: 0.75 },
  E: { jawOpen: 0.12, mouthPress: 0.2, mouthClose: 0.2 },
  F: { jawOpen: 0.25, mouthRollUpper: 0.45, mouthUpperUpLeft: 0.35, mouthUpperUpRight: 0.35 },
  G: { jawOpen: 0.02, mouthClose: 0.95, mouthPress: 0.35 },
  H: { jawOpen: 0.18, mouthLowerDownLeft: 0.5, mouthLowerDownRight: 0.5, mouthPress: 0.2 },
  L: { jawOpen: 0.25, mouthRollUpper: 0.45, mouthUpperUpLeft: 0.35, mouthUpperUpRight: 0.35 },
}
...
function Head({ url, viseme, gain, headRig }: { url: string; viseme: string; gain: number; headRig: 'scene'|'neck'|'head' }) {
  ...
  // ----- IDLE refs (extended) — also reused to drive speech shapes
  const idleTargetsRef = useRef<Array<{
    mesh:any;
    // brows
    idxBrowInner:number|null; idxBrowOuterL:number|null; idxBrowOuterR:number|null;
    ...
    idxMouthUpperUpL:number|null; idxMouthUpperUpR:number|null;
    idxMouthLowerDownL:number|null; idxMouthLowerDownR:number|null;
    // nose
    idxNoseSneerL:number|null; idxNoseSneerR:number|null;
    // jaw
    idxJawOpen:number|null;
  ...
  useEffect(() => {
    ...
    // IDLE + extended mouth shapes (used by idle and speech)
    const idleFound: typeof idleTargetsRef.current = []
    scene.traverse((o:any) => {
      if (!o.isMesh || !o.morphTargetDictionary || !o.morphTargetInfluences) return
      const d = o.morphTargetDictionary as Record<string, number>
  ...
  function animateMouth(dt: number) {
    // Stage 1: smooth the commanded openness
    const cmd = mouthCmdRef.current
    const tgtPrev = mouthTgtOpenRef.current
    const tc = cmd > tgtPrev ? ATTACK_TC : RELEASE_TC
    const alpha = 1 - Math.exp(-dt / Math.max(1e-3, tc))
    mouthTgtOpenRef.current = clamp01(tgtPrev + (cmd - tgtPrev) * alpha)

    // Stage 2: lerp current toward target (visual stability)
    const curPrev = mouthCurOpenRef.current
    const curNext = lerp(curPrev, mouthTgtOpenRef.current, LERP_MOUTH)
    mouthCurOpenRef.current = curNext

    // smile assist for wide vowels
    const smileAssist =
      mouthTgtOpenRef.current > 0.8 ? 0.25 :
      mouthTgtOpenRef.current > 0.5 ? 0.14 : 0.02

    // Primary mouth targets collected earlier
    for (const { mesh, idxMouthOpen, idxJawOpen, idxSmileL, idxSmileR } of mouthTargetsRef.current) {
      const infl = mesh.morphTargetInfluences as number[]
      if (idxMouthOpen != null) infl[idxMouthOpen] = curNext
      if (idxJawOpen   != null) infl[idxJawOpen]   = Math.min(1, Math.max(infl[idxJawOpen] ?? 0, curNext * 0.65))
      if (idxSmileL != null) infl[idxSmileL] = lerp(infl[idxSmileL] ?? 0, smileAssist, LERP_SMILE)
      if (idxSmileR != null) infl[idxSmileR] = lerp(infl[idxSmileR] ?? 0, smileAssist, LERP_SMILE)
    }

    // Extra ARKit-ish mouth shapes from recipe
    const v = visemeRef.current
    const recipe = VISEME_RECIPE[v] || VISEME_RECIPE['X']
    const scaleByOpen = (x:number) => clamp01(x * (0.65 + 0.35*curNext))

    for (const t of idleTargetsRef.current) {
      const infl = t.mesh.morphTargetInfluences as number[]

      if (t.idxMouthFunnel     != null) infl[t.idxMouthFunnel]     = scaleByOpen(recipe.mouthFunnel ?? 0)
      if (t.idxMouthPucker     != null) infl[t.idxMouthPucker]     = scaleByOpen(recipe.mouthPucker ?? 0)
      if (t.idxMouthPress      != null) infl[t.idxMouthPress]      = Math.max(infl[t.idxMouthPress] ?? 0, scaleByOpen(recipe.mouthPress ?? 0))
      if (t.idxMouthRollLower  != null) infl[t.idxMouthRollLower]  = scaleByOpen(recipe.mouthRollLower ?? 0)
      if (t.idxMouthRollUpper  != null) infl[t.idxMouthRollUpper]  = scaleByOpen(recipe.mouthRollUpper ?? 0)
      if (t.idxMouthShrugLower != null) infl[t.idxMouthShrugLower] = scaleByOpen(recipe.mouthShrugLower ?? 0)
      if (t.idxMouthShrugUpper != null) infl[t.idxMouthShrugUpper] = scaleByOpen(recipe.mouthShrugUpper ?? 0)
      if (t.idxSmileL          != null) infl[t.idxSmileL]          = Math.max(infl[t.idxSmileL] ?? 0, scaleByOpen(recipe.mouthSmileLeft ?? 0))
      if (t.idxSmileR          != null) infl[t.idxSmileR]          = Math.max(infl[t.idxSmileR] ?? 0, scaleByOpen(recipe.mouthSmileRight ?? 0))
      if (t.idxMouthStretchL   != null) infl[t.idxMouthStretchL]   = Math.max(infl[t.idxMouthStretchL] ?? 0, scaleByOpen(recipe.mouthStretchLeft ?? 0))
      if (t.idxMouthStretchR   != null) infl[t.idxMouthStretchR]   = Math.max(infl[t.idxMouthStretchR] ?? 0, scaleByOpen(recipe.mouthStretchRight ?? 0))

      if (t.idxJawOpen         != null && recipe.jawOpen != null)
        infl[t.idxJawOpen] = Math.max(infl[t.idxJawOpen] ?? 0, clamp01(recipe.jawOpen))

      // close / teeth together proxy
      if (t.idxMouthPress != null && recipe.mouthClose != null) {
        infl[t.idxMouthPress] = Math.max(infl[t.idxMouthPress] ?? 0, clamp01(recipe.mouthClose))
      }

      // upper/lower lip adjustments
      if (t.idxMouthUpperUpL   != null) infl[t.idxMouthUpperUpL]   = Math.max(infl[t.idxMouthUpperUpL] ?? 0, clamp01(recipe.mouthUpperUpLeft  ?? 0))
      if (t.idxMouthUpperUpR   != null) infl[t.idxMouthUpperUpR]   = Math.max(infl[t.idxMouthUpperUpR] ?? 0, clamp01(recipe.mouthUpperUpRight ?? 0))
      if (t.idxMouthLowerDownL != null) infl[t.idxMouthLowerDownL] = Math.max(infl[t.idxMouthLowerDownL] ?? 0, clamp01(recipe.mouthLowerDownLeft  ?? 0))
      if (t.idxMouthLowerDownR != null) infl[t.idxMouthLowerDownR] = Math.max(infl[t.idxMouthLowerDownR] ?? 0, clamp01(recipe.mouthLowerDownRight ?? 0))
    }
  }
